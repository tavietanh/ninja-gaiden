...
void EnemyMachineGun::Initialize()
{
	...
	sprite_alivemove = new CSpriteDx9(*SpriteManager::getInstance()->getSprite(eSpriteID::SPRITE_ENEMY_MACHINE_GUN_MOVE));
	sprite_shoot = new CSpriteDx9(*SpriteManager::getInstance()->getSprite(eSpriteID::SPRITE_ENEMY_MACHINE_GUN_SHOOT));
...
D3DXVECTOR3 EnemyMachineGun::GetStartPositionOfSkill(float x, float y)
{
...
void EnemyMachineGun::Update()
{
	...
	case STATE_SHOOTING:
		if (isShoot)
		{
				...
				if (m_TimeChangeDirectAttack > 300)
				{
					countBullet += 1;
					if (m_Direction == eDirection::LEFT)
					{
						SkillManager::getInstance()->addSkillIntoList(eIDTypeSkill::ENEMY_BULLET, GetStartPositionOfSkill(-10, 4), D3DXVECTOR2(-0.8f, 2.0f), 1);
					}
					else
					{
						SkillManager::getInstance()->addSkillIntoList(eIDTypeSkill::ENEMY_BULLET, GetStartPositionOfSkill(10, 4), D3DXVECTOR2(0.8f, 2.0f), 1);
