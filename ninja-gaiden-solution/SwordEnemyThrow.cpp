...
SwordEnemyThrow::SwordEnemyThrow()
{

}
SwordEnemyThrow::SwordEnemyThrow(D3DXVECTOR3 _position, eDirection _direction, eObjectID _objectID) : Skill(_position, _direction, _objectID)
{
	this->m_Position.z = 1.0f;
}

void SwordEnemyThrow::Initialize()
{
	this->m_Sprite = new CSpriteDx9(*SpriteManager::getInstance()->getSprite(eSpriteID::SPRITE_SWORD_ENEMY_THROW));
	this->m_Position.z = 0.0f;
	this->m_Physic->setVelocityX(1.0f);
	this->m_Physic->setAccelerate(D3DXVECTOR2(0.0f, 0.0f));
	this->m_TypeSkill = eIDTypeSkill::SWORD_ENEMY_THROW;
...
void SwordEnemyThrow::UpdateMovement()
{
